...
/*
 * add '#define FORSYTH_IMPLEMENTATION' before including to create the
 * implementation
 *
 * C99 compatibility
 * forsyth prefix
...
ForsythVertexIndexType *
forsythReorderIndices(ForsythVertexIndexType *outIndices,
                      const ForsythVertexIndexType *indices, int nTriangles,
                      int nVertices);
...
static ForsythScoreType
    forsythCachePositionScore[FORSYTH_CACHE_SCORE_TABLE_SIZE];
...
static void forsythInit() {
  for (int i = 0; i < FORSYTH_CACHE_SCORE_TABLE_SIZE; i++) {
    ...
    forsythCachePositionScore[i] =
        (ForsythScoreType)(FORSYTH_SCORE_SCALING * score);
  }
...
static ForsythScoreType forsythFindVertexScore(int numActiveTris,
                                               int cachePosition) {
  if (numActiveTris == 0) {
...
forsythReorderIndices(ForsythVertexIndexType *outIndices,
  ...
  ForsythAdjacencyType *numActiveTris =
      (ForsythAdjacencyType *)malloc(sizeof(ForsythAdjacencyType) * nVertices);
  ...
  ForsythArrayIndexType *offsets = (ForsythArrayIndexType *)malloc(
      sizeof(ForsythArrayIndexType) * nVertices);
  ForsythScoreType *lastScore =
      (ForsythScoreType *)malloc(sizeof(ForsythScoreType) * nVertices);
  ForsythCachePosType *cacheTag =
      (ForsythCachePosType *)malloc(sizeof(ForsythCachePosType) * nVertices);

  uint8_t *triangleAdded = (uint8_t *)malloc((nTriangles + 7) / 8);
  ForsythScoreType *triangleScore =
      (ForsythScoreType *)malloc(sizeof(ForsythScoreType) * nTriangles);
  ForsythTriangleIndexType *triangleIndices =
      (ForsythTriangleIndexType *)malloc(sizeof(ForsythTriangleIndexType) * 3 *
                                         nTriangles);
  ...
  ForsythTriangleIndexType *outTriangles = (ForsythTriangleIndexType *)malloc(
      sizeof(ForsythTriangleIndexType) * nTriangles);
  int outPos = 0;
  ...
  while (bestTriangle >= 0) {
    ...
    for (int i = 0; i < FORSYTH_VERTEX_CACHE_SIZE + 3; i++) {
      ...
      ForsythScoreType newScore =
          forsythFindVertexScore(numActiveTris[v], cacheTag[v]);
